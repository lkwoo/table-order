...
### UI 실기동 검증 (Browser E2E, this session, 2026-08-31)
사용자 요청으로 **실제 브라우저 화면**을 구동해 결과물을 육안 검증. 백엔드(uvicorn/SQLite :8000) + 프런트 dev 서버(Vite :5173, /api→백엔드 프록시)를 띄우고 Playwright(헤드리스 Chromium)로 UI를 조작하며 스크린샷 캡처(콘솔 에러 0).
- **고객 여정**: 테이블 로그인 → 메뉴 목록(카테고리 탭 메인/음료/주류, 7개 메뉴) → 메뉴 상세 모달(장바구니 담기) → 장바구니(수량 ±/삭제/총액) → 주문 확인("금액은 서버에서 재계산") → **주문 확정 → "주문이 접수되었습니다 #1, 20,000원"**.
- **관리자 여정**: 관리자 로그인 → 실시간 주문 현황 대시보드에 방금 주문이 **실시간 반영**(테이블 1 / 20,000원 / "#1 김치찌개 외 1건" / 대기중) → 테이블 상세(김치찌개×1 9,000 + 제육볶음×1 11,000 = 합계 20,000, 준비중/완료 전이·세션 종료·과거 내역) → 메뉴 관리(카테고리별 순서변경·수정·삭제·메뉴 등록).
- 고객 주문이 백엔드를 거쳐 관리자 화면까지 흐르는 전 경로를 화면으로 확인. 스크린샷은 로컬 `screenshots/`(gitignore).
- Node.js 24.19.0 + Playwright/Chromium(로컬 설치, `--no-save`)로 구동. **Docker 컨테이너 실기동만 여전히 미실행**(런타임 부재).
